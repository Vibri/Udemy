...
public class RemotePortScanner {
	...
	public static void main(String[] args) {
		...
		while (!isValid) {
			try {
				...
				if (fromPort >= 0 && fromPort <= 65536) // verify that port is in the valid range
				{
					...
					System.out.println("Invalid port! Port range is: 0 - 65536"); // inform user about the valid port
																					// range
				}
		...
		{
			try {
				Socket socket = new Socket(targetIp, port); // try to open on socket connection using the specified ip
															// address and the test port number
